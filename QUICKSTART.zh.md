...
「✨ 一键配图」：给每个没画面的段生成搜索词（装了 Ollama 就用本地模型，否则关键词提取），**立刻**从 Commons（历史画/地图/老照片）或 Pexels/Pixabay 搜图并下载，故事板马上显示缩略图——先出一版粗剪，再逐段替换不满意的。勾「已有画面的段也重配」可全部重来。60 段大约 5–7 分钟，后台跑，有进度条。
