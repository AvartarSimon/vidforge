...
## 0.1 分类（不同选题方向的一套设置，可选）

在新建项目页面可以选一个**分类**：一套跨项目复用的设置和参考资料——要讲什么、参考账号/资料源（含链接）、常用标签、发布平台（抖音/小红书/YouTube…）、过往心得，以及像"FND"这类需要把关的分类可以写禁用关键词/平台限制。选了分类后：新项目会带入它的默认声音/配音服务/字幕设置；第 1 步用"我的浏览器里的 AI 生成脚本"时，分类的定位说明、资料源、禁用词会自动附进提示词，让 AI 照着写、照着避雷。

分类存成 `~/.vidforge/categories/<id>.json` 的普通 JSON 文件，**不是数据库**——一个人单机用，没有并发写、没有关系查询的需求，普通文件反而更好：能直接拷走备份、能进 git 版本管理、能直接打开改。多机器同步或合并：新建项目页「管理分类…」里的导出/导入，或命令行 `vidforge category export/import`（导入默认合并、`--replace` 整体替换）。
...
「✨ 一键配图」：**给每一段配 3–10 张图**（约每 5 秒旁白一张，可调），并行下载，故事板边跑边出图。
  搜索词来自本地模型或浏览器里的大模型（下拉可选，后者准得多）。
  - **按内容的语言搜**：中文脚本先用中文搜（Openverse / Commons / 百度这类收录中文标注的源），
    中文找不够再用**翻译好的**英文去 Commons / Internet Archive 搜——而不是一上来就把中文改写成英文。
  - **每段多个关键词、按重要性排序**：模型为每段给出 3–4 个可拍成画面的具体主体（这段主要讲的排第一，
    细节排最后）。配图按这个顺序取，所以画面出现的次序和旁白讲的次序一致；前面的关键词配够了就不用后面的，
    某个关键词搜不到就顺延到下一个。
  - **宁缺毋滥**：只接受标题/描述里真的写着搜索词的图（相邻实词必须在标题里也相邻，否则"秦"+"毅"会match到秦刚和王毅的合影）；
    自动剔除 logo / 广告 / 海报 / 截图 / 带水印 的条目和付费图库域名（Shutterstock、Getty、视觉中国、千图…）。
  - 某一段旁白太抽象（"这里有个容易被误解的问题"）搜不到东西时，用**本片主题的素材**补足到下限，并在结果里点名，建议你自己换。
  - **模型核对切题**（默认开）：文字规则只能判断「词对上了」，判断不了「这张苏格兰山坡和秦国没关系」。
    勾上后本地模型会读这一段旁白，从候选标题里挑真正切题的，每段最多问 2 次。实测 23 段：
    关掉约 2.5 分钟，开着约 8.5 分钟，但配出来的是秦国青铜鼎、秦国墓、长平之战、战国金冠这类正确主体。
  - 装了本地视觉模型（`ollama pull qwen2.5vl:3b`）可勾「逐段视觉核对」，逐段抽查一张判断水印/内容；没有独显时每段约 1 分钟，默认关闭。
  - 下载的超大图（博物馆的 7000px TIFF 能有 120 MB）自动缩到 1920px，省磁盘也省渲染时间。
  - 手动选的图也一样：付费图库域名直接拒绝；其它图加入后在后台核对，有水印的在故事板上标「⚠ 水印/标注」。
  - **两轮制**：第一轮严格去重（每段拿到的图互不相同）；第一轮没配上的段，第二轮放宽来源和去重再试一次，
    所以正常情况下不会留下空白段落。进度条显示「x/23 段 · y 张 · 已用 / 约还需」。
  - **可以切到别的项目**：配图任务绑定在它启动时的那个项目上，切换项目不会打断它，也不会把图写错地方；
    项目切换在左侧栏「切换 / 新建项目」，和「历史版本」（同一个项目的时间点回退）是两回事。
  - 双击启动器时若发现之前有个旧版本的 vidforge 还在跑，会自动把它替换掉（以前会一直打开旧的那个，新功能报"not found"）。
...
不想为 ElevenLabs 付费也能"用文字描述一个声音"：[VoxCPM2](https://github.com/OpenBMB/VoxCPM)（Apache-2.0，可商用，支持中文+9种方言+共30种语言）。在**实际要渲染的那台机器**上装一次：
```
