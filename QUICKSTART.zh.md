...
「✨ 一键配图」：给每个没画面的段生成搜索词（装了 Ollama 就用本地模型，否则关键词提取），**立刻**从 Commons（历史画/地图/老照片）或 Pexels/Pixabay 搜图并下载，故事板马上显示缩略图——先出一版粗剪，再逐段替换不满意的。勾「已有画面的段也重配」可全部重来。后台跑，有进度条、预计剩余时间和「停止」按钮；**每配好一段就立刻写进项目并出现在故事板上**，中途停止或关页面，已经配好的都保留。
  - **宁缺毋滥**：只接受标题/描述里真的写着搜索词的图；装了本地视觉模型（`ollama pull qwen2.5vl:3b`，约 3 GB）时还会逐张核对"有没有水印/文字标注、画的是不是这个东西"，不过关的段留空，不塞不相关的图。视觉核对在没有独显的电脑上每张约 1 分钟，可以取消勾选。
  - 手动选的图也一样：付费图库/带水印站点（Shutterstock、Getty、视觉中国、千图…）直接拒绝；其它图加入后在后台核对，有水印的在故事板上标「⚠ 水印/标注」，你换一张即可。
