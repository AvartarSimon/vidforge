...
vidforge 不是时间线式的剪辑软件。它是一条流水线（有一个简单的本地网页界面，见第 1.5 步）：**你写一个 `project.json`（每段旁白 + 每段配什么画面），它输出成片 `final.mp4` + 字幕 + 封面。**
...
## 第 1.5 步：用网页界面（推荐日常这样用）

命令行只需要记这一条，其余都在页面里点：

```powershell
vidforge ui my-first-video        # 启动本地服务并自动打开浏览器 http://127.0.0.1:8765
```
...
```
┌ 左：段列表 ──────────────────────────────────┐ ┌ 右 ─────────────────────────┐
│ [缩略图]  id  章节名  运镜  停顿              │ │ 标题 / 封面文字 / 声音 / 语速  │
│ [▶试听 ↑ ↓ ✕]  旁白文本框                     │ │ 配音服务 / 字幕                │
│ [音频播放器] 8.0 s  · 文字已改，配音需重新试听 │ │ [▶ 渲染] [全部试听] 状态 用时   │
│ 画面: 本地图片|Pexels 图片|本地视频|Pexels 视频|动画 │ │ 渲染日志（实时）              │
│   路径 [选择文件…]  /  搜索词 [获取/换一张]     │ ├──────────────────────────────┤
│   （动画段：组件下拉 + props JSON）             │ │ 成片播放器 · 下载 mp4/srt/封面  │
│ ＋ 添加一段                                    │ │ 封面预览 · 章节列表            │
└──────────────────────────────────────────────┘ └──────────────────────────────┘
```

- **改文字**：直接在文本框里改，0.7 秒后自动保存到 `project.json`（右上角"已保存"）。保存前会做校验，非法内容（比如图片路径不存在）会红条提示且**不写盘**。
- **换图**：画面类型选"本地图片"→"选择文件…"从电脑挑一张（会复制进 `assets\`）；或选"Pexels 图片/视频"→ 输英文搜索词 →"获取"；不满意点"换一张"。
- **试听**：每段一个"▶ 试听"，只合成这一段（几秒），播放器就地出现；文字改过后会标黄"配音需重新试听"。"全部试听"一次合成所有段。
- **动画段**：类型选"动画（Remotion）"，选组件，props 直接编辑 JSON（有语法错误会提示且不保存）。
- **渲染**：右侧"▶ 渲染"，日志实时滚动；完成后成片播放器、章节、下载链接自动出现。渲染中再点会提示"已有渲染在进行"。
- **多语言**：右上角切换语言（`en` / `zh`…），左边文本框就变成该语言的旁白，标题/声音/语速等改的是该语言的 `variants`；没翻译的段会在顶部红条列出，但列表照常可编辑——直接在中文视图里把中文填进去就行。URL 带 `?lang=zh` 可直接打开中文视图。
- 它和命令行改的是同一个 `project.json`，两边混用没有冲突；只限本机访问（127.0.0.1）。
