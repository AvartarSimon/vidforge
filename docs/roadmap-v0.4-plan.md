...
| **P3** | 3.2 更多素材源（Openverse、Internet Archive） · 3.4 学习变体（双语字幕 + 词汇卡） | 素材面和多平台 | 3 天 |
| **P3.5 真人素材库 + 口型同步** | `assets/me/` 你在各场景录的沉默镜头/说话镜头 + 标签；段上 `{"me": {"tags": [...], "talking": false}}` 自动挑未用过的匹配片段作画中画/全屏；`talking: true` 走口型同步 provider（Sync Labs API / HeyGen；开源 MuseTalk、LatentSync 需 NVIDIA GPU）并自动勾披露 | 用户提议：一次录库，每次像新录；80% 用不说话镜头（零风险），20% 用口型替换 | 2–3 天 + API/GPU |
