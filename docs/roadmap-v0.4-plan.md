...
| **P2.5 真实性层 ✅ 9-17（按用户调整：不用原声、不真录）** | 画中画叠加层（图/视频/主持人，位置/大小/时机/滑入淡入）✅ · **`Host` 风格化数字主持人**（Remotion，口型由配音包络驱动，眨眼微动，发型/肤色/衣服/眼镜/胡须/署名可配；本地免费、无需披露）✅ · HeyGen 逼真分身 provider（需 key，自动勾选合成内容披露；未实测） · **ElevenLabs Voice Design 品牌声音**（文字描述→3 候选→保存为 voice_id；不克隆真人；未实测） · 发布页「AI 使用声明」（YouTube `containsSyntheticMedia` 自动判断 + 中英声明文案进简介）✅ | 应对 YPP 与国内 AI 标识法规：原创 + 一致的人格 + 合规披露 | 待补：基于用户照片的分层插画主持人；真人录像段 `keep_audio`（用户暂不需要） |
| **UI/UX 与本地智能 ✅ 9-17** | 左侧导航 + 顶栏 + AI 抽屉的新布局；四套主题（系统/亮/暗/护眼）；保存按钮 + 整份历史（30 份）+ **每段版本历史** + **每段成片按哈希缓存**；项目页（新建/最近/切换）；`vidforge start` / `vidforge shortcut`；Ollama 本地模型层（关键词、译搜索词、章节名、侧栏问答） | 用户要求：好看、能存、易启动、能和 AI 对话、有本地智能 | 完成 |
| **P3 ✅ 9-17** | 3.2 更多素材源（Openverse、Internet Archive） · 3.4 学习变体（双语字幕 + 词汇卡） | 素材面和多平台 | 完成 |
| **P3.5 真人素材库 + 口型同步 ✅ 9-17** | `vidforge/me.py`：`~/.vidforge/me/` 或 `assets/me/`，文件名即标签（含 talking/说话 = 说话镜头），`pick()` 按标签选**用得最少**的未用过片段。项目里 `{"me": {"tags": [...], "talking": false}}` 可加在 clip 或 overlay 上；沉默镜头直接当普通视频用（零风险），说话镜头走 `vidforge/lipsync.py`（`synclabs` sync.so API / `musetalk` 本地命令，均未实测，需 key/GPU）并自动置 `disclosure.realistic_presenter`。UI 第 3 步新增「🎥 我的镜头」tab（筛选/上传/改标签/加为主画面或画中画），CLI `vidforge me scan\|tag` | 用户提议：一次录库，每次像新录；80% 用不说话镜头（零风险），20% 用口型替换 | 完成；Sync Labs/MuseTalk 未接真实 key/GPU 实测 |
